    ...
    for attack_tool_type in attack_tool_types:
        for llm in llms:
            for attack_type in attack_types:
                ...
                base_cmd = f'''nohup python -u main_attacker.py --llm_name {llm} --attack_type {attack_type} --use_backend {backend} --attacker_tools_path {attacker_tools_path} --res_file {log_file}_{suffix}_vic.csv'''
                ...
                cmd = f"{base_cmd}{specific_cmd} > {log_file}_{suffix}_vic.log 2>&1 &"
                
                print(f'{log_file}_{suffix}_vic.log')
                os.system(cmd)
