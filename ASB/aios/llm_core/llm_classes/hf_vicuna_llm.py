...
class HfVicunaLLM(BaseLLM):
    ...
    def load_llm_and_tokenizer(self) -> None:
        ...
        # ================== 新增下面这两行 ==================
        # 拦截空字典 {}，将其还原为 None，让 Hugging Face 自动探测显存
        ...
        self.model = MODEL_CLASS[self.model_type].from_pretrained(
            self.model_name,
            device_map="auto",
            max_memory=self.max_gpu_memory,  # 现在这里传入的是纯正的 None 了
        ...
        # ================== 修改位置：适配 Vicuna-1.5 ==================
        # 如果模型没有自带聊天模板，或者当前加载的是 vicuna 模型，则手动设置 Vicuna 专用模板
        if self.tokenizer.chat_template is None or "vicuna" in self.model_name.lower():
    ...
    def process(self,
        ...
        if self.context_manager.check_restoration(agent_process.get_pid()):
            ...
            prompt = self.tokenizer.apply_chat_template(
                messages,
                tokenize = False,
                add_generation_prompt = True # 新增这一行：确保在 prompt 末尾追加 'ASSISTANT:'
            )

            input_ids = self.tokenizer.encode(prompt, return_tensors="pt")

            attention_mask = input_ids != self.tokenizer.pad_token_id
            # 直接使用模型所在的设备，彻底避免设备不匹配
