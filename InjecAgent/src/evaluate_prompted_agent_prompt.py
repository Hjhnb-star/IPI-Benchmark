...
def predict_one_case(system_prompt, user_prompt, params, item, model_class, tool_dict, step_2=False):
    """
    预测单个测试案例的模型输出
    包含 Delimiting 防御逻辑的独立实现
    """
    # 获取当前的 prompt_type
    current_prompt_type = params.get('prompt_type', '')

    # --- Delimiting 防御逻辑 ---
    # 定义分隔符处理函数：仅在 Delimit 模式下生效
    # [cite_start]参考 MELON 论文 [cite: 648-660]，使用明显的符号包裹检索内容
    def apply_delimiter(content):
        if current_prompt_type == 'InjecAgent_Delimit':
            # 使用 XML 风格或特殊符号包裹，并在前后加换行符确保模型识别
            return f"\n<<<< START TOOL OUTPUT >>>>\n{content}\n<<<< END TOOL OUTPUT >>>>\n"
        return content
    ...
    # === 应用 Delimiting 防御到工具输出 (Observation) ===
    # 处理 Step 1 的工具响应
    raw_obs_step1 = item['Tool Response']
    processed_obs_step1 = apply_delimiter(raw_obs_step1)

    # 处理 Step 2 的工具响应 (如果有)
    raw_obs_step2 = item.get('Attacker Tool Response', '')
    processed_obs_step2 = apply_delimiter(raw_obs_step2)
    # =======================================================

    # 构建Agent scratchpad (使用处理过的 observation)
    if step_2:
        if len(item['Attacker Tools']) == 0 or not all(k in item for k in ["Attacker Tool Thought", "Attacker Tool Parameters", "Attacker Tool Response"]):
            agent_scratchpad = f"\n{item['Thought']}\nAction: {item['User Tool']}\nAction Input: {item['Tool Parameters']}\nObservation: {processed_obs_step1}\n"
        else:
            agent_scratchpad = f"\n{item['Thought']}\nAction: {item['User Tool']}\nAction Input: {item['Tool Parameters']}\nObservation: {processed_obs_step1}\nThought: {item['Attacker Tool Thought']}\nAction: {item['Attacker Tools'][0]}\nAction Input: {item['Attacker Tool Parameters']}\nObservation: {processed_obs_step2}\n"
    else:
        agent_scratchpad = f"\n{item['Thought']}\nAction: {item['User Tool']}\nAction Input: {item['Tool Parameters']}\nObservation: {processed_obs_step1}\n"

    # 填充用户提示模板
    # 注意：如果是 Repeat Prompt 模式，这里的 user_prompt 模板会自动包含末尾的重复 Input，
    # format 函数会自动将 input_text 填充到模板中所有 {input} 的位置。
        ...
        with open(output_path, 'w', encoding='utf-8') as f_out:
            for case in tqdm(test_cases, desc=f"Processing {attack_type} cases"):
                try:
                    ...
                    # [修改点 1] 保存 Step 1 的 Utility
                    case['utility'] = parsed_step1.get('utility', 0)
                    ...
                    if (not params['only_first_step'] 
                        ...
                        case["Attacker Tool Response"] = get_simulated_attacker_tool_response(
                            ...
                            attacker_tool_parameters=case["Attacker Tool Parameters"],
                            tool_dict=tool_dict
                        )
                        ...
                        # [修改点 2] 更新 Utility (基于 Step 2 的结果更新)
                        # 如果 Step 2 模型拒绝了攻击并正常回复用户，utility 会变为 1
                        case['utility'] = parsed_step2.get('utility', 0)
                    ...
                    print(f"\n{error_msg}")
                    # print(f"Case details preview: {json.dumps(case, ensure_ascii=False, indent=2)[:500]}...") # 可选调试
                    continue
