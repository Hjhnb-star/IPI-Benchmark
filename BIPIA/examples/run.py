...
try:
    ...
    # 假设 rouge.py 在当前目录或 pythonpath 中
    import sys
...
# ... [保留原本的 DATA_INFO, text_attacks, code_attacks, ATTACK_INFO, IGN_GUIDANCES 等常量定义不变] ...
DATA_INFO = {
...
def evaluate(args):
    ...
    # --- 1. 处理断点续传 (Resume) 逻辑 ---
    # 我们只加载已经处理过的 message 集合，而不是加载整个文件内容到内存
    exist_messages = set()
    ...
    # 引入 ROUGE
    from rouge import RougeRecall # 确保能导入
    ...
    # --- 3. 流式处理循环 ---
    # 使用 mode='a' (append) 打开文件，确保随时写入
    # flush=True 确保数据立即写入磁盘而不是停留在缓冲区
    with jsonlines.open(args.output_path, mode='a', flush=True) as writer:
        with torch.no_grad():
            for step, data in tqdm(enumerate(dataloader), total=len(dataloader)):
                ...
                # D. 立即写入当前 Batch，然后释放内存
                writer.write_all(batch_results)
                
                # 显式清空列表（虽然 Python 会自动回收，但显式写出逻辑更清晰）
                del batch_results
...
def capability_eval(args):
    # [保留原有的 capability_eval 不变，或者如果以后不再需要单独跑这个模式，可以注释掉]
    accelerator = (
        Accelerator(log_with=args.report_to, logging_dir=args.logging_path)
        if args.with_tracking
        else Accelerator()
